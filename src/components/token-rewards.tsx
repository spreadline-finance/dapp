...
function RewardValue({ value, unit, rewardRaw, className }: { value: string; unit?: string; rewardRaw?: string | null; className?: string }) {
  const context = useContext(RewardPriceContext);
  const usd = rewardRaw !== undefined && context ? rewardUsdValue(rewardRaw, context.decimals, context.quote, context.asset, context.now) : null;
  if (!usd) return <strong className={className}>{value}{unit && <> <small>{unit}</small></>}</strong>;
  return <span className={`rewards-value-stack${className ? ` ${className}` : ""}`}><strong>{usd} USD</strong><span>{value}{unit ? ` ${unit}` : ""}</span></span>;
}
function Metric({ label, value, unit, note, rewardRaw }: { label: string; value: string; unit?: string; note?: ReactNode; rewardRaw?: string | null }) {
  return <div className="rewards-metric">{rewardRaw !== undefined && unit ? <span className="rewards-metric-label"><SpreadTokenIcon size={16}/>{label}</span> : <span>{label}</span>}<RewardValue value={value} unit={unit} rewardRaw={rewardRaw}/>{note && <p>{note}</p>}</div>;
}
function PersonalRewards({ report, wallet, stale, now, readFailed }: { report: RewardsReport; wallet: WalletState; stale: boolean; now: number; readFailed: boolean }) {
  ...
  if (!wallet.account || !personal) return <aside className="rewards-panel rewards-wallet-prompt"><span className="rewards-prompt-icon"><SpreadTokenIcon size={31}/></span><div><span className="rewards-eyebrow">YOUR WALLET</span><h2>Your rewards start here</h2><p>{!wallet.account ? "Connect your wallet to see your estimated rewards, pending payments and holdings." : "Your wallet’s balances and allocations are missing from this report. Refresh to check again."}</p><WalletButton wallet={wallet}/><span className="rewards-prompt-footnote">Automatic payments · No claim needed</span></div></aside>;
  return <section className="rewards-panel rewards-participation">
    ...
    <h2>Your rewards</h2>
    <p>Rewards are sent periodically to this wallet every 5 minutes when your allocation is ready. No claim transaction needed.</p>
      ...
      <div className="rewards-personal-summary"><div className="rewards-personal-total"><span><SpreadTokenIcon size={17}/>Earned · awaiting payment</span><RewardValue className="rewards-personal-value" value={amount(personal.pending, decimals)} unit={asset} rewardRaw={personal.pending}/></div><div className="rewards-received-total"><span><SpreadTokenIcon size={17}/>Received so far</span><RewardValue className="rewards-received-value" value={amount(personal.paid, decimals)} unit={asset} rewardRaw={personal.paid}/></div></div>
...
function PaymentHistory({ report, account }: { report: RewardsReport | null; account: string | null }) {
  ...
  return <section className="rewards-panel rewards-history"><div className="rewards-heading"><div><h2>Your confirmed payments</h2><p>Recent transfers reported as confirmed, with receipts you can inspect.</p></div><Check size={21}/></div>
    {personal.receipts.map((receipt) => <div className="rewards-receipt" key={`${receipt.transactionHash}:${receipt.epochId}`}><div><RewardValue value={amount(receipt.amount, report?.rewardAsset.decimals)} unit={report?.rewardAsset.symbol} rewardRaw={receipt.amount}/><p>Distribution #{receipt.epochId} · Block {receipt.blockNumber}{receipt.confirmedAt ? ` · ${time(receipt.confirmedAt)}` : ""}</p></div><a className="rewards-text-button" href={`${EXPLORER}/tx/${receipt.transactionHash}`} target="_blank" rel="noreferrer">View receipt<ArrowUpRight size={13}/></a></div>)}
  </section>;
}
...
export function TokenRewards({ wallet, now }: { wallet: WalletState; now: number }) {
  ...
  const scheduleLabel = "Every 5 minutes";
  const statusLabel = snapshot.isPending ? "Reading payout service" : snapshot.data?.status === "unconfigured" ? "Payout setup pending" : !report ? "Payout data unavailable" : stale ? "Last available service report" : report.statusReason === "payment-pending" ? "Transaction awaiting confirmation" : report.status === "attention" ? "Payout service delayed" : report.executionMode === "report-only" ? "Reporting connected · payouts not running" : report.status === "ready" ? "Periodic rewards reporting" : "Payouts paused";
  ...
  return <RewardPriceContext.Provider value={{ quote: currentPrice, asset, decimals, now }}><div className="rewards-workspace">
    ...
    <section className="rewards-distribution-summary" aria-labelledby="distribution-totals-title">
      ...
      <div className="rewards-summary-grid">
        <Metric label="Distributed so far" value={amount(pool?.paid, decimals)} rewardRaw={pool?.paid} unit={asset} note="Confirmed holder payments recorded by the payout service."/>
      ...
      <p className="rewards-summary-note">{stale ? `Last update: ${time(report.updatedAt)}. These amounts may have changed; a fresh report is needed to confirm current totals.` : "Pending amounts are not guaranteed to arrive in the next run. Allocation, minimum payouts and network confirmations still apply."}</p>
    </section>
    ...
    <details className="rewards-panel rewards-policy rewards-pool-breakdown"><summary><span>Pool breakdown</span><span>Allocations &amp; developer share</span><ChevronDown size={16}/></summary><div className="rewards-overview"><section className="rewards-panel"><div className="rewards-heading"><div><h2>Pool overview</h2><p>75% of received creator fees goes to eligible holders.</p></div><Coins size={22}/></div>
      <div className="rewards-total"><span>Confirmed paid to holders</span><RewardValue value={amount(report?.totals.paidToHolders, decimals)} unit={asset} rewardRaw={report.totals.paidToHolders}/><p>Completed payments reported by the payout service. Pending allocations are shown separately.</p></div>
      <div className="rewards-metrics">
    ...
    <details className="rewards-panel rewards-policy"><summary><span>How rewards work</span><span>75% holders · 25% developer</span><ChevronDown size={16}/></summary><div className="rewards-heading"><div><h2>One fee pool. Two shares.</h2><p>The split applies to creator fees received, before allocating each holder’s proportional share. The fee receiver’s token holdings also participate in the 75% holder pool, in addition to its 25% developer share.</p></div><span className="rewards-tag">Fixed 75 / 25</span></div>
      ...
      <div className="rewards-schedule"><div><span><CalendarClock size={14}/>Distribution cadence</span><strong>{scheduleLabel}</strong><p>Rewards are sent periodically when available fees, funding and minimum payout conditions are met.</p></div><div><span><Clock3 size={14}/>Next distribution check</span><strong>{nextRun}</strong><p>{rewardsServiceMessage(report, stale)}</p></div><div><span><Coins size={14}/>Payout asset</span><strong>{report ? report.rewardAsset.symbol : "Set at token launch"}</strong><p>Rewards are paid in the fee asset: ETH or USDG.</p></div></div>
