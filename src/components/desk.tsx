...
import { EXPLORER, TRACKED_SYMBOLS, USDG, type StockAsset } from "@/lib/market-types";
...
function current(time: string | undefined | null, now: number) { return !!time && now > 0 && Date.parse(time) <= now + 60000 && now - Date.parse(time) < 90000; }
...
function PoolHistory({ symbol, now, pools }: { symbol: string; now: number; pools: DeskPoolBoard | undefined }) {
  ...
  const history = useQuery({ queryKey: ["desk-history", symbol], queryFn: ({ signal }) => getData<DeskHistory>(`desk/history?symbol=${encodeURIComponent(symbol)}`, signal), staleTime: 60000, refetchInterval: (q) => pollingInterval(60000, q.state.error, q.state.fetchFailureCount) });
  ...
  return <section className="earn-history" aria-label={`${symbol} observed pool spreads`}>
    <div className="earn-section-heading"><div><h3>Watch the spread change</h3><p>Saved pool spot prices against the issuer midpoint.</p></div><span className="earn-source-label">{history.data?.status === "ready" ? `${available.length} observations` : "Recorded observations"}</span></div>
...
function MarketWatch({ assets, now, onAnalyze, registryError }: { assets: StockAsset[]; now: number; onAnalyze: (symbol: string) => void; registryError?: string | null }) {
  ...
  const pools = useQuery({ queryKey: ["desk-pools", symbol], queryFn: ({ signal }) => getData<DeskPoolBoard>(`desk/pools?symbol=${encodeURIComponent(symbol)}`, signal), enabled: !!asset, staleTime: 15000, refetchInterval: (q) => pollingInterval(15000, q.state.error, q.state.fetchFailureCount, q.state.data?.dataStatus?.retryAt) });
  ...
  const referenceFresh = ref && current(ref.generatedAt, now) && !ref.halted;
  const watchlist = TRACKED_SYMBOLS.filter((tracked) => assets.some((item) => item.symbol === tracked));
  return <>
    <section className="earn-market-board">
      <div className="earn-section-heading"><div><h2>Live pool desk</h2><p>The same stock token can trade at different prices across pools.</p></div><button className="earn-button earn-button-quiet" disabled={pools.isFetching || wait > 0 || !asset} onClick={() => void pools.refetch()}><RefreshCw size={14} className={pools.isFetching ? "spin" : ""}/>{wait > 0 ? `Retry in ${wait}s` : pools.isFetching ? "Updating" : "Refresh"}</button></div>
      <div className="earn-market-toolbar"><div className="earn-watchlist" aria-label="Stock watchlist">{watchlist.map((tracked) => <button key={tracked} aria-pressed={symbol === tracked} onClick={() => setSelection(tracked)}>{tracked}</button>)}</div><label className="earn-asset-select"><span>Stock token</span><select value={symbol} onChange={(event) => setSelection(event.target.value)} disabled={!assets.length}>{!assets.length && <option value={selection}>Loading assets…</option>}{assets.map((item) => <option key={item.address} value={item.symbol}>{item.symbol} · {item.name}</option>)}</select></label></div>
...
export function StrategyDesk({ assets, now, wallet, onAnalyze, registryError }: { assets: StockAsset[]; now: number; wallet: WalletState; onAnalyze: (symbol: string) => void; registryError?: string | null }) {
  ...
  const snapshot = useQuery({ queryKey: ["desk", wallet.account ?? "public"], queryFn: ({ signal }) => getData<DeskSnapshot>(`desk${wallet.account ? `?address=${encodeURIComponent(wallet.account)}` : ""}`, signal), staleTime: 15000, refetchInterval: (q) => pollingInterval(15000, q.state.error, q.state.fetchFailureCount, q.state.data?.dataStatus?.retryAt) });
  ...
  return <TokenLogoScope stocks={assets}><div className="earn-workspace">
    ...
    {data?.status === "unconfigured" && <div className="earn-launch-state"><div className="earn-launch-icon"><Clock3 size={20}/></div><div><h3>The desk is open. The vault is not launched yet.</h3><p>Explore real pool prices below. Deposits and earnings become available after the vault is deployed and connected; no income has been generated by this desk yet.</p></div><button className="earn-text-button" onClick={() => setTab("method")}>Explore the mechanism <ArrowRight size={15}/></button></div>}
