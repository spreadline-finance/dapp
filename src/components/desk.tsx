...
export function StrategyDesk({ assets, now, wallet, onAnalyze, registryError }: { assets: StockAsset[]; now: number; wallet: WalletState; onAnalyze: (symbol: string) => void; registryError?: string | null }) {
  ...
  return <TokenLogoScope stocks={assets}><div className="earn-workspace">
    ...
    </div> : <section className="earn-vault-unavailable"><Vault size={24}/><div><h2>{snapshot.isPending ? "Checking vault availability…" : data?.status === "unconfigured" ? "The vault has not launched" : "Vault accounting is unavailable"}</h2><p>{data?.status === "unconfigured" ? "Deposits and rewards open when the USDG vault is connected. You can explore pool prices below." : "Vault balances and wallet actions appear when verified accounting is available. Pool market watch is available below."}</p></div><button className="earn-button earn-button-quiet" onClick={() => setTab("method")}>How it works<ArrowRight size={14}/></button></section>}
