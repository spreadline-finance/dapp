  ...
  return <TokenLogoScope stocks={assets}><MotionConfig reducedMotion="user"><div className="lending-workspace lending-refined lending-polished">
    ...
    {tab === "markets" ? <>
      ...
      <section className="lending-comparison" aria-label="Morpho lending market comparison">
        ...
        {!visible.length && <div className="lending-empty"><Landmark size={30}/><h3>{markets.isPending ? "Reading Morpho markets…" : markets.error ? "Market data is reconnecting" : "No markets match these filters"}</h3><p>{stockOnly && !unlisted ? "This snapshot may only have unlisted Stock Token markets. Include unlisted markets to research them." : "Adjust the asset or search to explore the available markets."}</p></div>}
