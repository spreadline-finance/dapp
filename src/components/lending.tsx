...
const sourceOld = (date: string | null, now: number) => !sourceIsCurrent(date, now, 5 * 60000);
...
export function Lending({ assets, now, wallet }: { assets: StockAsset[]; now: number; wallet: WalletState }) {
  ...
  const markets = useQuery({ queryKey: ["lending-markets"], queryFn: ({ signal }) => getData<LendingMarkets>("lending/markets", signal), enabled: tab === "markets" || overviewOpen || recoveredTarget?.kind === "market", staleTime: 120000, refetchInterval: (q) => pollingInterval(120000, q.state.error, q.state.fetchFailureCount, q.state.data?.dataStatus?.retryAt) });
  const vaults = useQuery({ queryKey: ["lending-vaults"], queryFn: ({ signal }) => getData<LendingVaults>("lending/vaults", signal), enabled: tab === "vaults" || overviewOpen || recoveredTarget?.kind === "vault", staleTime: 120000, refetchInterval: (q) => pollingInterval(120000, q.state.error, q.state.fetchFailureCount, q.state.data?.dataStatus?.retryAt) });
  ...
  function openPosition(target: LendingTarget) { lastReviewButton.current = document.activeElement instanceof HTMLButtonElement ? document.activeElement : null; setSelectedId(""); setSelectedVaultAddress(""); setRecoveredTarget({ ...target, initialOperation: target.initialOperation ?? "withdraw" }); }
  ...
  const refreshAt = Math.max(retryAt, active.dataUpdatedAt + 10000);
  ...
  const refreshLabel = active.isFetching ? "Updating lending data" : refreshWait > 0 ? `Refresh available in ${refreshWait} seconds` : "Refresh lending data";
  return <TokenLogoScope stocks={assets}><MotionConfig reducedMotion="user"><div className="lending-workspace lending-refined lending-polished">
    ...
    <details className="lending-overview" onToggle={(event) => setOverviewOpen(event.currentTarget.open)}><summary><ProtocolLogo protocol="morpho" size={22}/><span>Morpho market overview</span><ArrowDown size={14}/></summary><div className="lending-summary">
      ...
      <div><ShieldCheck size={20}/><span>Curated vaults</span><strong>{vaults.data ? vaults.data.vaults.length : "—"}</strong><small>{vaults.data ? `${vaults.data.vaults.filter(depositDisabled).length} with deposit restrictions / alerts` : vaults.error ? "Availability unavailable" : "Reading availability"}</small></div>
    </div></details>
    <div className="lending-toolbar" id="lending-explore"><div className="lending-tabs" aria-label="Lending views">
      {(["markets", "vaults"] as const).map((item) => <button key={item} aria-pressed={tab === item} onClick={() => setTab(item)}>{item === "markets" ? "Markets" : "Vaults"}{item === "vaults" && vaults.data && <span className="lending-count">{vaults.data.vaults.length}</span>}{tab === item && <motion.span className="lending-tab-line" layoutId={reduceMotion ? undefined : "lending-tab"} transition={{ duration: .22, ease: "easeOut" }}/>}</button>)}
    </div><button className="lending-refresh" aria-label={refreshLabel} title={refreshLabel} disabled={active.isFetching || refreshWait > 0} onClick={() => void active.refetch()}><RefreshCw size={16} className={active.isFetching ? "spin" : ""}/><span>{active.isFetching ? "Updating" : refreshWait > 0 ? `Refresh in ${refreshWait}s` : "Refresh"}</span></button></div>
    {active.data && (active.error || active.data.dataStatus) && <p className="lending-caution" role="status"><CircleHelp size={17}/>Showing the last snapshot. Rates and liquidity may have changed. {active.error?.message ?? active.data.dataStatus?.reason}</p>}
    ...
    {tab === "markets" ? <>
      ...
      <div className="lending-toggles"><label><input type="checkbox" checked={stockOnly} onChange={(e) => setStockOnly(e.target.checked)}/>Stock Token collateral</label><label><input type="checkbox" checked={unlisted} onChange={(e) => setUnlisted(e.target.checked)}/>Include unlisted markets</label><span>{markets.data ? `${visible.length} markets` : markets.error ? "Unavailable" : "Loading…"}</span>{(search || stockOnly || unlisted || loanAddress !== USDG) && <button className="lending-reset" onClick={() => { setSearch(""); setStockOnly(false); setUnlisted(false); setLoanAddress(USDG); }}>Clear filters<X size={12}/></button>}</div>
      ...
      <section className="lending-comparison" aria-label="Morpho lending market comparison" aria-busy={markets.isPending}>
        {!!visible.length && <div className="lending-table-scroll" tabIndex={0} role="region" aria-label="Compare lending markets">
        ...
        {!visible.length && <div className="lending-empty" role="status"><Landmark size={30}/><h3>{markets.isPending ? "Loading Morpho markets…" : markets.error && !markets.data ? "Market data is unavailable" : !rows.length ? "No markets are available in this snapshot" : "No markets match these filters"}</h3><p>{markets.isPending ? "Fetching current rates, liquidity and collateral from Morpho." : markets.error && !markets.data ? markets.error.message : !rows.length ? "Check again after the next refresh." : stockOnly && !unlisted ? "This snapshot may only have unlisted Stock Token markets. Include unlisted markets to research them." : "Adjust the asset or search to explore the available markets."}</p></div>}
      ...
      <section className="lending-comparison" aria-label="Curated lending vaults" aria-busy={vaults.isPending}>{!!vaults.data?.vaults.length && <div className="lending-table-scroll" tabIndex={0} role="region" aria-label="Compare curated lending vaults"><table className="lending-data-table" role="table"><thead><tr><th scope="col">Lending vault</th><th scope="col" className="lending-secondary-column">Total supplied</th><th scope="col">Liquidity</th><th scope="col">Net base APY</th><th scope="col">Availability</th><th scope="col"><span className="sr-only">Review vault</span></th></tr></thead><tbody>
      ...
      {!vaults.data?.vaults.length && <div className="lending-empty" role="status"><Landmark size={30}/><h3>{vaults.isPending ? "Loading listed vaults…" : vaults.error && !vaults.data ? "Vault data is unavailable" : "No listed vaults are available in this snapshot"}</h3><p>{vaults.isPending ? "Fetching vault rates, liquidity and deposit availability from Morpho." : vaults.error && !vaults.data ? vaults.error.message : "Explore individual lending markets, or check again after the next refresh."}</p></div>}
    ...
    <div className="lending-source"><span><ProtocolLogo protocol="morpho" size={18}/>Morpho · Robinhood Chain</span><span>{active.data ? `Fetched ${ageLabel(active.data.fetchedAt, now)}` : active.error ? "Data unavailable" : "Connecting to Morpho’s indexer"}</span><a href="https://docs.morpho.org/developers/api/get-started/" target="_blank" rel="noreferrer">Source<ArrowUpRight size={12}/></a></div>
    ...
    <Dialog.Root open={!!activeTarget} onOpenChange={(open) => { if (!open) { setSelectedId(""); setSelectedVaultAddress(""); setRecoveredTarget(undefined); } }}>
      <Dialog.Portal><Dialog.Overlay className="lending-dialog-overlay"/><Dialog.Content className={`lending-dialog lending-workspace lending-dialog-polished ${selected || selectedVault ? "has-insights" : ""}`} onOpenAutoFocus={(event) => { event.preventDefault(); dialogTitle.current?.focus(); }} onCloseAutoFocus={(event) => { event.preventDefault(); if (lastReviewButton.current?.isConnected) lastReviewButton.current.focus(); else document.getElementById("lending-positions-heading")?.focus(); }}>
        <div className="lending-dialog-heading"><div className="lending-dialog-identity"><TokenPairLogo supply={activeTarget?.asset} collateral={activeTarget?.collateral} size={44}/><div><span className="eyebrow">{activeTarget?.kind === "market" ? "MORPHO MARKET" : "MORPHO VAULT"}</span><Dialog.Title ref={dialogTitle} tabIndex={-1}>{activeTarget?.kind === "market" && activeTarget.asset ? `${activeTarget.asset.symbol} supply market` : activeTarget?.label}</Dialog.Title><Dialog.Description>{activeTarget?.collateral ? `${activeTarget.collateral.symbol} collateral · Morpho on Robinhood Chain` : "Morpho on Robinhood Chain"}</Dialog.Description></div></div><Dialog.Close className="lending-dialog-close" aria-label="Close lending review"><X size={20}/></Dialog.Close></div>
