...
export default {
  async fetch(
    ...
    if (url.pathname === "/api/rewards" || url.pathname.startsWith("/api/rewards/")) {
      if (url.pathname !== "/api/rewards") return json({ error: "Endpoint not found." }, 404);
      if (request.method !== "GET") return json({ error: "Only read-only GET requests are supported." }, 405);
      try {
        const allowed = ["address", "before"];
        ...
        const rawEpoch = url.searchParams.get("before");
        if (rawEpoch !== null && !/^[1-9]\d{0,15}$/.test(rawEpoch)) throw new Error("Invalid distribution number.");
        const budget = await consumeBudget(env.SPREADLINE_DB, request.headers.get("cf-connecting-ip") || "local", "/api/rewards", 60);
        ...
        return json(await createRewardsService(env).snapshot(account, rawEpoch === null ? undefined : BigInt(rawEpoch)));
      } catch (error) {
        console.error(JSON.stringify({ event: "rewards_api_failed", reason: error instanceof Error ? error.name : "unknown" }));
        return json({ error: "The rewards report is unavailable. Check the requested wallet, pagination and service configuration." }, 400);
      }
    }
